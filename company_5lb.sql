...
-- ДЗ 1. Дізнайтеся, які клієнти були зареєстровані в нашій компанії (показати всі доступні поля). Відсортуйте список за Прізвищем.
SELECT
*
FROM
customer
...
-- ДЗ 2. Вивести унікальні назви виробників (manufacture ) з таблиці продуктів в одному запиті, впорядкованому за алфавітом.
SELECT DISTINCT
manufacture
FROM
product
...
-- ДЗ 3. Отримати коротку інформацію про продукти (назва_продукту, виробник, категорія, тип_продукту, ціна), 
...
-- ДЗ 4. Отримати інформацію про клієнтів-жінок 1990-2000 років народження (ім'я, прізвище, стать, дата народження, номер телефону) 
...
-- ДЗ 5. Отримати інформацію з таблиці товарів про наявні на складі ноутбуки, які оснащені дисковими накопичувачами об'ємом 512 ГБ.
SELECT
*
FROM
  product
...
-- ДЗ 6. Отримати інформацію з таблиці товарів про наявні на складі ноутбуки або настільні комп'ютери, які оснащені дисковими накопичувачами 512 ГБ або 1 ТБ.
SELECT
*
FROM
  product
...
-- ДЗ 7. Отримати інформацію з таблиці рахунків-фактур (invoice ) про всі покупки, зроблені неавторизованими покупцями (customer_id NULL).
SELECT 
*
FROM
invoice
...
MySQL Aliases.
Бізнес потреба: Проаналізувати штат працівників компанії та та вивести інформацію про працівників що займають керівні посади.
Реалізація:
*/
USE company;
SELECT
...
Добавити в вибірку інформацію про працівників
...
employment_date AS 'Hire Date’ 
MySQL Self Join.
Бізнес потреба: Проаналізувати штат працівників компанії та та вивести інформацію про працівників та їхніх менеджерів що займають керівні посади.	
*/
...
e.maneger_id "Employee Manager ID",
...
Бізнес потреба: Проаналізувати штат працівників компанії та та вивести інформацію про працівників та департаменти в яких вони працюють.
Для реалізації поставленої задачі застосуємо MySQL Equijoin.	
Реалізація:
*/
SELECT 
e.employee_id "Employee ID",
...
Бізнес потреба: Проаналізувати штат працівників компанії та та вивести інформацію про працівників та Здійснені ним продажі.
Для реалізації поставленої задачі застосуємо MySQL INNER JOIN.
Реалізація:
*/
SELECT 
e.employee_id "Employee ID",
...
transaction_moment 'Transaction moment'
Бізнес потреба: Проаналізувати штат працівників компанії та та вивести інформацію про працівників та Здійснені ним продажі.
Для реалізації поставленої задачі застосуємо MySQL NATURAL JOIN.	
Реалізація:
*/
SELECT 
e.employee_id "Employee ID",
...
Бізнес потреба: Проаналізувати інформацію про продажі компанії.
Для реалізації поставленої задачі застосуємо MySQL JOIN.	
Реалізація:
*/
SELECT 
e.employee_id "Employee ID",
...
/*Завдання 5.7: Показати список працівників які здійснювали продажі
 для неавторизованих клієнтів.
Деталі: Отримати інформацію з таблиць employee,  invoice, customer про продажі.
Змінити попередній запит так щоб вивести інформацію 
про продажі  в яких відсутня авторизація клієнтів.
Бізнес потреба: Проаналізувати штат працівників компанії та та вивести інформацію про продажі.
Для реалізації поставленої задачі застосуємо MySQL JOIN.	
Реалізація:
*/
SELECT
...
Бізнес потреба: Проаналізувати штат працівників компанії та та вивести інформацію про Працівників, Менеджерів та Департаменти.
Для реалізації поставленої задачі застосуємо MySQL JOIN.
Реалізація:
*/
SELECT
...
LEFT JOIN 
employee as m
ON e.maneger_id = m.employee_id;

/*Завдання 5.9: Показати список працівників консультантів.
 Деталі: Цей запит виводить список всіх співробітників,
 які можуть / не можуть консультувати.
Бізнес потреба: Проаналізувати штат працівників компанії та вивести інформацію про осіб які можуть / не можуть консультувати.
Для реалізації поставленої задачі застосуємо MySQL UNION Operator.	
Реалізація:
*/
SELECT
employee_id,
...
/* Домашня 1.Відобразити список усіх замовлених найменувань товарів разом з іменами та прізвищами клієнтів 
і моментом проведення транзакції по інвойсу, відсортуйте дані за ідентифікатором замовлення Orders ID. 
...
/* Домашня 2.Відобразити імена співробітників, які працюють у відділі "Меркурій", і список всіх товарів, 
замовлених з '2023-07-01' по '2023-10-01’. відсортуйте дані за ідентифікатором замовлення Orders ID. 
Перелік полів: 
orders.orders_id AS 'Orders ID', 
product.product_name AS 'Product name',
product.category AS 'Product category',
...
SELECT
  ...
  employee.last_name AS 'Employee last name',
  employee.first_name AS 'Employee first name',
  department.department_name  AS 'Department Mercury'
FROM
  orders
  INNER JOIN product ON orders.product_id = product.product_id
  LEFT JOIN invoice ON orders.invoice_id = invoice.invoice_id
  LEFT JOIN employee 
  USING (employee_id)
...
/* Домашня 3.Показати імена та прізвища всіх клієнтів з таблиці клієнтів, а також клієнтів без замовлень і замовлень без клієнтів 
(якщо такі є).відсортуйте дані за ID інвойсу.
Перелік полів: 
...
/* 6 Лабораторна Вбудовані функції
Created by: Mikaellla
...
Для реалізації поставленої задачі застосуємо MySQL String Functions:
LPAD, CONCAT, CONCAT_WS, LOWER */
...
Для реалізації поставленої задачі застосуємо MySQL Date Functions:
 FORMAT, DATE_FORMAT, DATEDIFF, TIMESTAMPDIFF, YEAR
*/
SELECT 
first_name 'First name',
last_name 'Last name',
...
Деталі: Отримати інформацію з таблиці employee: 1) Визначити найвищу та найнижчу зарплату в компанії. 
2) Визначити середню премію для співробітників, які отримують премію. 
3) Визначити середнєзначення премії для всієї компанії.
Для реалізації поставленої задачі застосуємо MySQL Aggregate (Numeric) Functions:
...
Деталі: Отримати інформацію з таблиць department, employee : 1) Визначити кількість відділів компанії. 2) Визначити кількість міст, в яких компанія має представництва. 3. Визначити кількість менеджерів, продавців і т.д. 4. Показати кількість працівників у кожному відділі. 5. З'ясувати, скільки менеджерів, продавців і т.д. є в кожному відділі.
Перелік полів:
Кількість офісів;
...
Для реалізації поставленої задачі застосуємо MySQL GROUP BY Statement :
...
department_id As
'Department id',
COUNT(*) AS 'Count of employee by Department'
FROM employee
...
COUNT(*) AS 'Count of employee by Position for each Department'
FROM employee
...
Для реалізації поставленої задачі розглянемо MySQL HAVING Clause:
 COUNT(*), GROUP BY, HAVNG 
 */
 SELECT 
 department_id,
 position,
...
/*Завдання 6.5.3: Відобразити інформацію про список посад співробітників за відділами та представництвами. 
Деталі: Відобразити інформацію про список посад співробітників відділу продажів по відділах. 
...
/*ДЗ 6.1 Відобразити інформацію про товари (ідентифікатор товару, назву товару, категорію). 
Перелік полів: Ідентифікатор "Product ID" повинен формуватися з ідентифікатора співробітника з додаванням "0" 
довжиною 4 символи. Назва товару як "Назва товару" повинна містити назву виробника, не містити номер моделі 
(без крапки після "/" в полі product_name), формат: [виробник] :: [назва_продукту]. Категорія в полі "Категорія" повинна мати формат:
...
/*ДЗ 6.2 Відобразити інформацію про пропродажі по місячно. Місяць "Month" повинен формуватися з місяця в якому здійснювалися транзакції 
з додаванням "0" довжиною 2 символи. Загальна виручка as 'Total revenue' повинна містити загальну суму продажей за кожен місяць.
Квартал продажів as 'Sales Period' повинен містити порядковий номер кварталу в році і відповідний рік, формат: 
'Quater ' [квартал] - [рік]. Будь ласка, відсортуйте дані за датою продажу.
*/
USE company;
SELECT
...
GROUP BY LPAD(MONTH(order_datetime), 2, '0'),
         CONCAT('Quarter ', QUARTER(order_datetime), ' - ', YEAR(order_datetime))
ORDER BY YEAR(order_datetime), MONTH(order_datetime);

/*ДЗ 6.3 Відображення аналітичної інформації про продажі. 
